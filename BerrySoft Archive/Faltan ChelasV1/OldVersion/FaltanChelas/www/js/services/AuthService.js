...
AuthServices.factory("Auth",function($http,$q,AuthToken){
	...
	AuthFactory.Login = function(email,password){
		...
		var Url = "http://104.131.24.52:3000/authenticate";
		//var Url = "http://localhost:8080/authenticate";
		//var Url = "http://192.168.0.2:8080" + "/authenticate";
...
AuthServices.factory("AuthToken", function($window){
	...
	AuthTokenFactory.GetToken = function(){
		return $window.localStorage.getItem("token-App-FC");
	}

	AuthTokenFactory.SetToken = function(token){

		if(token){
			$window.localStorage.setItem("token-App-FC",token);
		}else{
			$window.localStorage.removeItem("token-App-FC");
		}
	}
