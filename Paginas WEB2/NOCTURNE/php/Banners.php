...
if (isset ($_POST['submit'])) 
{
	...
	&& in_array($extension, $allowedExts)) {
		...
		if ($_FILES["imagen"]["error"] > 0) {
	    	...
	    	if (file_exists("../files/Banners/" . $_FILES["imagen"]["name"])) {    //HERE!!!!


	    	} else {

	      		move_uploaded_file($_FILES["imagen"]["tmp_name"],
	      		"../files/Banners/" . $_FILES["imagen"]["name"]);    //HEREEE!!!!!

	    	}

	    	$photo = "http://nocturneoax.com/files/Banners/" . $_FILES["imagen"]["name"];  //HEREE!!!!!
